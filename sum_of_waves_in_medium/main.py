...
def update(frame):
    ...
    if frame >= green_starts:
        green_dot.set_data((frame-green_starts)*c_reduced + L/2, 1)  # c_reduced*dt is the distance covered in one timestep by the green dot
        blue_dot.set_data((frame - green_starts) * c_phase + L / 2, 1)  # c_reduced*dt is the distance covered in one timestep by the blue dot, position slightly lower
    return [vacuum_sum_line] + medium_sum_lines + [red_dot, green_dot, blue_dot]
