...
def init():
    ...
    return line_vacuum, line_medium,
...
def wavepacket_medium(x, t):
    sum_medium = 0
    for index in range(1, medium_n + 1):
        a_n = cos_fourier_component(initial_wavepacket, index)
        b_n = sin_fourier_component(initial_wavepacket, index)
        phase = material_constant * angular_frequency / (
                    resonant_angular_frequency ** 2 - angular_frequency ** 2 + 0.012) * (x_range / 2) / speed_of_light
